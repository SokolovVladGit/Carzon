-- Carzon — VIN Phase 2F: persist successful NHTSA vPIC decodes into listing_vin_source_results.
--
-- * Extends complete_vin_decode_job_success only; same RPC signature and privileges.
-- * Upserts when p_provider_id = 'nhtsa_vpic' (fake/carzon test decoder unchanged).
-- * No full VIN, vin_hash, verbatim provider bodies, URLs, or credentials in source results.
-- * Status: succeeded if at least one useful catalog field; else partial (decode OK, thin data).

------------------------------------------------------------------------------
-- 1 — complete_vin_decode_job_success (+ Phase 2F NHTSA taxonomy upsert)
------------------------------------------------------------------------------
...
comment on function public.complete_vin_decode_job_success(
    ...
    'listing_vin_source_results (basic_decode, owner visibility). Does not store VIN, '
    'vin_hash, or unredacted provider bodies in source results.';
