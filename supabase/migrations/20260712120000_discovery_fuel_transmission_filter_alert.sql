-- Carzon — filter-alert criteria parity for fuelType and transmissionType.
--
-- Mirrors Flutter discovery filters on listings.fuel_type and
-- listings.transmission_type. Missing/empty JSON keys impose no constraint.
-- DB wire values (e.g. dual_clutch) match listing_discovery_criteria_json.dart.
...
comment on function public.listing_matches_saved_discovery_criteria(public.listings, jsonb) is
    'JSON criteria vs listings row — mirrors feed filters incl. fuelType/transmissionType; free-text search matches title OR make OR model; sort ignored; internal only.';
