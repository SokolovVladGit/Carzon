-- Carzon — VIN create/update RPC explicit decode enqueue.
--
-- Root cause: listing_vehicle_identity AFTER INSERT trigger alone is not
-- sufficient when identity rows are written from PostgREST RPC sessions
-- (trigger may not run). Phase 2A backfill only covered identities that
-- existed at migration time; later creates could persist identity without
-- vin_processing_jobs / listing_vin_report_snapshot.
--
-- Fix: after successful identity write in create_listing_v2 and
-- update_listing_details_v2, explicitly call carzon_enqueue_vin_decode_from_identity
-- (idempotent; no plaintext VIN exposure). One-time backfill for orphaned identities.
...
------------------------------------------------------------------------------
-- One-time backfill: identities without snapshot (orphaned enqueue gap)
------------------------------------------------------------------------------
