-- Carzon — listing discovery free-text search: title OR make OR model.
--
-- Parity with Flutter feed search (`ListingsQuery.search` via PostgREST `.or`).
-- Filter-alert saved criteria `search` uses the same semantics.
-- Out of scope: city, description, full-text search.
...
comment on function public.listing_matches_saved_discovery_criteria(public.listings, jsonb) is
    'JSON criteria vs listings row — mirrors feed filters; free-text search matches title OR make OR model; sort ignored; internal only.';
