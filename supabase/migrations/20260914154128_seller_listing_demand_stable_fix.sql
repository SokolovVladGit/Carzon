-- Carzon — Phase 5B repair: keep demand RPCs STABLE.
--
-- Drops the seller-profile write helper from read-only demand functions.
-- Professional gate reads seller_profiles only; missing row or non-dealer
-- → 42501. Demand semantics unchanged.
...
comment on function public.get_my_seller_listing_demand() is
    ...
    'auth.uid() only. Point-in-time. Read-only STABLE. Does not create '
    'seller_profiles.';
...
comment on function public.get_my_seller_inventory_demand() is
    ...
    'Read-only STABLE. Zero active listings → visible zero.';
