-- Carzon — VIN Phase 2K: buyer-visible NHTSA basic_decode as public_summary.
--
-- * Replaces complete_vin_decode_job_success NHTSA branch to write visibility = public_summary.
-- * One-time UPDATE promotes existing nhtsa_vpic basic_decode owner rows (succeeded/partial).
-- * No new providers, table grants, VIN/vin_hash exposure, or raw payload columns.
...
comment on function public.complete_vin_decode_job_success(
    ...
    'Worker-only: updates decode cache, listing snapshot, and for NHTSA vPIC only upserts '
    'listing_vin_source_results (basic_decode, public_summary visibility for buyer-safe RPC). '
    'Does not store VIN, vin_hash, or unredacted provider bodies in source results.';

------------------------------------------------------------------------------
-- 2 — Promote existing NHTSA basic_decode rows to buyer-visible
------------------------------------------------------------------------------

update public.listing_vin_source_results
   set visibility = 'public_summary',
       updated_at = now()
 where source_id = 'nhtsa_vpic'
   and confidence = 'basic_decode'
   and visibility = 'owner'
   and status in ('succeeded', 'partial');

------------------------------------------------------------------------------
-- 3 — Privileges unchanged (service_role only; re-assert after replace)
------------------------------------------------------------------------------
