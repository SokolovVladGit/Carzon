-- Carzon — seller-entered transmission_type on listings.
--
-- Optional structured gearbox field (distinct from drivetrain / VIN decode).

alter table public.listings
    add column if not exists transmission_type text;

alter table public.listings
    drop constraint if exists listings_transmission_type_chk;

alter table public.listings
    add constraint listings_transmission_type_chk
        check (
...
comment on column public.listings.transmission_type is
    'Seller-entered gearbox type: manual|automatic|cvt|robotic|dual_clutch|other. Distinct from drivetrain and VIN decode.';
