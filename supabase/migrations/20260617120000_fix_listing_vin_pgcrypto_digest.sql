-- Carzon — Fix VIN Phase 1 hash: digest() not resolved under RPC search_path.
--
-- Root cause: SECURITY DEFINER listing RPCs use
--   set search_path = public, pg_temp
-- so unqualified digest(...) from pgcrypto is invisible when pgcrypto lives in
-- schema extensions (Supabase default). The literal algorithm also resolves as
-- unknown without ::text.
--
-- Fix: enable pgcrypto in extensions when missing; add internal SQL helper with
-- search_path public, extensions, pg_temp; RPCs delegate SHA-256 hex hashing.
-- Product semantics unchanged (same vin_hash algorithm).

create extension if not exists pgcrypto with schema extensions;

create or replace function public.carzon_sha256_hex_utf8(p_plain text)
returns text
language sql
immutable
parallel safe
...
as $$
  select encode(digest(convert_to(p_plain, 'UTF8'), 'sha256'::text), 'hex');
$$;

revoke all on function public.carzon_sha256_hex_utf8(text) from public;
revoke all on function public.carzon_sha256_hex_utf8(text) from anon;
revoke all on function public.carzon_sha256_hex_utf8(text) from authenticated;

------------------------------------------------------------------------------
-- Recreate listing RPCs (signatures, grants, VIN semantics unchanged)
------------------------------------------------------------------------------
