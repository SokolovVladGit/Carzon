...
Deno.serve(async (req: Request): Promise<Response> => {
  try {
    ...
    const expectedSecret = Deno.env.get("CARZON_PROCESS_FILTER_ALERT_NOTIFICATIONS_SECRET");
    const header = req.headers.get("x-carzon-internal-secret");
    if (!expectedSecret || header !== expectedSecret) {
      console.warn("process-filter-alert-notifications: unauthorized invoke attempt");
    ...
    if (claimError) {
      console.error("claim_filter_alert_notification_events_for_processing", claimError);
    ...
    for (const event of events) {
      if (event.event_type !== "filter_alert_listing_match") {
      ...
      const { data: prefRow } = await supabase
        ...
        .select("global_enabled, filter_alerts_enabled")
        .eq("user_id", event.recipient_user_id)
        .maybeSingle();

      const globalOn = prefRow?.global_enabled === true;
      const filterAlertsOn = prefRow?.filter_alerts_enabled === true;
      if (!globalOn || !filterAlertsOn) {
        await supabase.from("notification_delivery_events").update({
          status: "skipped",
          processed_at: new Date().toISOString(),
          locked_at: null,
      ...
      for (const row of tokens) {
        const copy = filterAlertNotificationCopyForLocale(
    ...
    console.error("process-filter-alert-notifications fatal", e);
