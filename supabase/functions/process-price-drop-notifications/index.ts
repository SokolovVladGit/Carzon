 ...
 *
 * Claims only `price_drop_favorite` via `claim_price_drop_notification_events_for_processing`.
 * Does not send price amounts, listing title, seller contact, or other private fields.
 */
...
import {
  ...
  PRICE_DROP_VALIDATION_FAILED,
  priceDropNotificationDataPayload,
} from "../_shared/price_drop_favorite_validation.ts";
import { priceDropNotificationCopyForLocale } from "../_shared/push_notification_copy.ts";
...
Deno.serve(async (req: Request): Promise<Response> => {
  try {
    ...
    const expectedSecret = Deno.env.get("CARZON_PROCESS_PRICE_DROP_NOTIFICATIONS_SECRET");
    const header = req.headers.get("x-carzon-internal-secret");
    if (!expectedSecret || header !== expectedSecret) {
      console.warn("process-price-drop-notifications: unauthorized invoke attempt");
    ...
    if (claimError) {
      console.error("claim_price_drop_notification_events_for_processing", claimError);
    ...
    for (const event of events) {
      if (event.event_type !== "price_drop_favorite") {
        await supabase.from("notification_delivery_events").update({
          status: "skipped",
          processed_at: new Date().toISOString(),
          locked_at: null,
          last_error: "unexpected_event_type",
        }).eq("id", event.id);
        continue;
      }
      ...
      if (
        ...
        await supabase.from("notification_delivery_events").update({
          ...
          processed_at: new Date().toISOString(),
          locked_at: null,
          last_error: "invalid_recipient_or_owner",
        }).eq("id", event.id);
        continue;
      }
      ...
      const { data: prefRow } = await supabase
        ...
        .select("global_enabled, price_drops_enabled")
        .eq("user_id", event.recipient_user_id)
        .maybeSingle();

      const globalOn = prefRow?.global_enabled === true;
      const priceDropsOn = prefRow?.price_drops_enabled === true;
      if (!globalOn || !priceDropsOn) {
        await supabase.from("notification_delivery_events").update({
          status: "skipped",
          processed_at: new Date().toISOString(),
          locked_at: null,
      ...
      if (listingErr) {
        ...
        await requeueOrFail(supabase, event, PRICE_DROP_VALIDATION_FAILED);
        continue;
      }
      ...
      const { data: favoriteRow, error: favoriteErr } = await supabase
        .from("favorites")
        .select("user_id")
        .eq("user_id", event.recipient_user_id)
      ...
      if (favoriteErr) {
        console.error("favorites select", favoriteErr);
        await requeueOrFail(supabase, event, PRICE_DROP_VALIDATION_FAILED);
        continue;
      }
      ...
      if (!favoriteRow) {
        await supabase.from("notification_delivery_events").update({
          ...
          processed_at: new Date().toISOString(),
          locked_at: null,
          last_error: FAVORITE_MISSING_SKIP_REASON,
        }).eq("id", event.id);
        continue;
      }
      ...
      const stringData = priceDropNotificationDataPayload(event);
      let anySuccess = false;
      let lastErr = "";

      for (const row of tokens) {
        const copy = priceDropNotificationCopyForLocale(
    ...
    console.error("process-price-drop-notifications fatal", e);
