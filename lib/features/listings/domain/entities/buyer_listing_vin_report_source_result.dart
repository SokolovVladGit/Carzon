...
/// Buyer-safe row from `get_listing_vin_report_for_buyer` (sanitized projection only).
class BuyerListingVinReportSourceResult extends Equatable {
  ...
  static BuyerListingVinReportSourceResult? tryParse(Map<String, dynamic> row) {
    try {
      final sid = _trim(row['source_id']);
      if (sid == null) return null;
      return BuyerListingVinReportSourceResult(
        sourceId: sid,
...
/// Outcome of buyer VIN report RPC (non-throwing at datasource).
class BuyerListingVinReportLookupResult extends Equatable {
  const BuyerListingVinReportLookupResult({
    this.results = const [],
