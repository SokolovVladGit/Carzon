...
Listing _listing({required String make, required String model}) => Listing(
  id: 'l1',
  title: 'Test',
  make: make,
  model: model,
...
void main() {
  ...
  testWidgets('spec rows tolerate very long labels and wrapped values', (
    tester,
  ) async {
    final longSuffix = '${'У' * 90} (${'Я' * 90})';
    await tester.pumpWidget(
      app(
        _listing(
          make:
              '$longSuffix — производитель очень очень очень очень очень очень очень очень очень длинное поле марки автомобиля',
          model: '$longSuffix — модель с неудобными переносами и символами ···',
        ),
      ),
    );
