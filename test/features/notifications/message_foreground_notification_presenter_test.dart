...
class _RecordingDisplay implements MessageForegroundNotificationDisplay {
  ...
  Future<void> showMessageForegroundNotification(String conversationId) async {
    ...
    titles.add(MessageNotificationPublicCopy.title);
    bodies.add(MessageNotificationPublicCopy.body);
  }

  @override
  Future<void> showFilterAlertForegroundNotification(String listingId) async {
    shownListingIds.add(listingId);
    titles.add(FilterAlertNotificationPublicCopy.title);
    bodies.add(FilterAlertNotificationPublicCopy.body);
  }
}
...
''',
    ...
    opened.add(RemoteMessage(data: {'type': 'message', 'conversation_id': okId}));
...
''',
    ...
    expect(display.titles, everyElement(MessageNotificationPublicCopy.title));
    expect(display.bodies, everyElement(MessageNotificationPublicCopy.body));
    expect(
      display.titles.join(),
      isNot(contains('@')),
...
''',
  ...
  test('valid foreground filter_alert shows generic Russian copy only', () async {
    dotenv.testLoad(
      fileInput: '''
SUPABASE_URL=https://x.supabase.co
SUPABASE_ANON_KEY=anon
...
''',
    ...
    expect(display.shownConversationIds, isEmpty);
    expect(display.titles, [FilterAlertNotificationPublicCopy.title]);
    expect(display.bodies, [FilterAlertNotificationPublicCopy.body]);
