...
const _sqlPath = 'test/supabase/seller_listing_demand_behavior.sql';
...
void main() {
  test('Seller listing demand Phase 5B behavior against local fixtures', () async {
    final available = await _localPostgresAvailable();
    if (!available) {
      // ignore: avoid_print
      print(
        'SKIP live seller listing demand SQL fixtures: local Supabase Postgres '
        'is not reachable via docker exec supabase_db_carzon, or the Phase 5B '
