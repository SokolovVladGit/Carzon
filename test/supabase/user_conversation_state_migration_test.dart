...
void main() {
  group('20260518100000_user_conversation_state.sql', () {
    ...
    test('creates user_conversation_state with expected columns and PK', () {
      ...
      expect(lower.contains('create table if not exists public.user_conversation_state'), isTrue);
    ...
    test('defines participant-scoped select policy', () {
      final lower = sql.toLowerCase();
      expect(lower.contains('user_conversation_state_participant_select'), isTrue);
      expect(lower.contains('c.buyer_id = auth.uid()'), isTrue);
      expect(lower.contains('c.seller_id = auth.uid()'), isTrue);
    });

    test('mark_conversation_read is security definer and granted to authenticated', () {
      final lower = sql.toLowerCase();
      expect(lower.contains('create or replace function public.mark_conversation_read'), isTrue);
    ...
    test('get_unread_conversation_count is security definer and counts distinct conversations', () {
      final lower = sql.toLowerCase();
      expect(
        lower.contains('create or replace function public.get_unread_conversation_count'),
        isTrue,
      );
