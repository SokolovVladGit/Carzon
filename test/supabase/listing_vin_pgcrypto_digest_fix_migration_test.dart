  ...
  group('20260617120000_fix_listing_vin_pgcrypto_digest.sql', () {
    ...
    setUpAll(() {
      ...
      expect(
        f.existsSync(),
        isTrue,
        reason: 'VIN digest fix migration must exist',
      );
    ...
    test(
      'defines internal carzon_sha256_hex_utf8 with extensions-capable search_path',
      () {
    ...
    test('RPCs delegate VIN hash to helper (no bare digest in RPC bodies)', () {
      expect(
        sql,
        contains('v_vin_hash := public.carzon_sha256_hex_utf8(v_vin_norm);'),
      );
    ...
    test('helper body uses digest with explicit algorithm type', () {
      final helperStart = sql.indexOf(
        'create or replace function public.carzon_sha256_hex_utf8',
      );
    ...
    test('does not add plaintext vin column to public.listings', () {
      ...
      for (final m in listingAlter.allMatches(sql)) {
        ...
        expect(standaloneVinCol.hasMatch(stmt), isFalse);
      }
    });

    test(
      'create_listing_v2 drop signature includes trailing p_vin text type',
      () {
        expect(
          sql.toLowerCase(),
          contains(
