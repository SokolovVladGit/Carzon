...
void main() {
  group('20260523120000_filter_alert_settings.sql', () {
    ...
    test('creates filter_alert_settings with expected shape', () {
      final lower = sql.toLowerCase();
      expect(
        lower.contains('create table if not exists public.filter_alert_settings'),
        isTrue,
      );
      expect(
        lower.contains(
    ...
    test('defines updated_at trigger consistent with migrations', () {
      final lower = sql.toLowerCase();
      expect(
        lower.contains('touch_filter_alert_settings_updated_at'),
        isTrue,
      );
      expect(
    ...
    test('enables RLS and authenticated-scoped CRUD policies', () {
      final lower = sql.toLowerCase();
      expect(
        lower.contains('alter table public.filter_alert_settings enable row level security'),
        isTrue,
      );
      ...
      expect(lower.contains('grant select, insert, update, delete on public.filter_alert_settings to authenticated'), isTrue);
    });
  });
}
